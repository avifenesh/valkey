...
#
# These are TDD tests -- the feature does not exist yet. Tests that require
# the unimplemented config will be skipped until the feature lands.
...
# Wait until an instance finishes sync (master_link_status:up).
...
start_cluster 2 2 {tags {external:skip cluster} overrides {cluster-node-timeout 3000}} {
    ...
    # ===== CONFIG DISABLED (does NOT need the feature config) =====
    ...
    test "CONFIG DISABLED - Normal full sync when feature is off" {
        ...
        # Feature disabled (or not implemented) -- don't set the config.
        # Just do a normal CLUSTER REPLICATE to primary 0.
        ...
        # Primary 0 SHOULD have done BGSAVE (normal full sync).
    ...
    test "HAPPY PATH - Sync from sibling replica avoids BGSAVE on primary" {
        ...
        if {![is_sync_from_replica_implemented]} {
            skip "repl-prefer-sync-from-replica config not yet implemented"
        }
        ...
        # Primary 0 should NOT have done a new BGSAVE (sync_full unchanged).
        assert_equal $p0_sync_full_before [get_info 0 sync_full] \
            "Primary sync_full should not increase -- no BGSAVE on primary"

        # Sibling 2 SHOULD have done a BGSAVE (sync_full increased by 1).
    ...
    # ===== WRITES DURING SYNC (catches C2: pending_repl_data drain) =====
    ...
    test "WRITES DURING SYNC - keys written to P during sibling RDB are not lost" {
        ...
        # Slow down sibling's BGSAVE so the RDB takes a while,
        # giving us a window to write data that must be buffered.
        ...
        # These writes go to P's replication stream, which N must buffer
        # (in pending_repl_data) and drain after the RDB loads.
        ...
        # Use longer timeout since the slow BGSAVE + 1000 key writes takes time.
        ...
        # ALL 1000 keys written during sync must be present on node 3.
        R 3 readonly
        for {set i 0} {$i < 1000} {incr i} {
    ...
    # ===== CONCURRENT WRITES — thorough before+during+after (buffer drain) =====
    ...
    test "RESIDUAL STATE - normal sync works after sync-from-replica completed" {
        ...
        # Node 3 just finished sync-from-replica (previous test). Verify healthy.
        ...
        # Primary SHOULD have done BGSAVE (normal sync, feature off).
        assert {[get_info 0 sync_full] > $p0_sync_full_before}
    ...
    test "REATTACH CYCLE - detach and reattach to different primary after sync-from-replica" {
        ...
        # This proves the state machine is clean after sync-from-replica.
        detach_node3
    ...
    # ------------------------------------------------------------------
    # AUTO-FAILOVER BLOCKED — node with incomplete data cannot be promoted
    # ------------------------------------------------------------------
    test "AUTO-FAILOVER BLOCKED - sibling sync blocks automatic failover" {
        if {![is_sync_from_replica_implemented]} {
        ...
        # Manually set the guard flag to simulate mid-sync state.
        # We can't easily trigger a real mid-sync pause, but we can
        # verify the INFO field blocks failover eligibility.
        set primary0_id [R 0 cluster myid]
        R 3 cluster replicate $primary0_id

        # After replicate completes, the flag should be cleared.
        wait_replica_of 3 $primary0_id
        wait_node_synced 3

        # Verify feature completed (flag cleared).
    ...
    test "FAILOVER BLOCKED - CLUSTER FAILOVER rejected during sibling sync" {
        ...
        # Node 3 is now a replica of primary 0 (from previous test).
        # Verify that CLUSTER FAILOVER works normally when not syncing.
        # We don't actually execute failover — just verify the error path
        # is wired up by checking the code handles the flag.
        assert_equal 0 [get_info 3 sync_from_replica_in_progress] \
            "Should not be syncing from sibling"
    }
